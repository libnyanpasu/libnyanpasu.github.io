...
const HalfCircleSVG = ({ className, ...props }: ComponentProps<typeof CircleSVG>) => {
  return <CircleSVG className={cn("w-[200%]", className)} {...props} />;
};

const HalfCircleContainer = ({ className, ...props }: ComponentProps<"div">) => {
  return (
    <div className={cn("relative h-full w-1/2 shrink-0 overflow-hidden", className)} {...props} />
  );
};
...
export function CircularProgress({
  ...
  return (
    ...
    >
      {indeterminate ? (
        <div className="absolute inset-0 animate-spin" data-slot="circular-progress-indeterminate">
          <div
      ...
      {children && (
        <div className="absolute inset-0 flex items-center justify-center text-xs">{children}</div>
      )}
    </div>
  );
}
