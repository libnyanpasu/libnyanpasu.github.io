...
Clash Nyanpasu currently supports:

- Windows 10 and later
- macOS 12.6 and later
- Linux (`glibc` >= 2.39)
...
- `amd64` / `x86_64` / `x64`: For popular 64-bit Intel/AMD processor devices.
- `aarch64` / `arm64`: For modern 64-bit Arm architecture devices (e.g., Apple Silicon chips, modern Arm-based Windows or Linux devices).

Currently, we only support the 64-bit architectures listed above; 32-bit and legacy Arm devices are not supported.
...
<Tabs>
  <TabItem label="Stable (need to compile)">
...
```
  ...
  <TabItem label="Development (need to compile)">
