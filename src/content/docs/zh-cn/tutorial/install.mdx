...
:::caution[Note]
Please note that Tauri does not officially **support** the portable variant. The portable variant provided by Clash Nyanpasu is generated through an automated packaging process and explicitly ignores WebView2 detection, so some problems may be encountered. We plan to stick to the principle that the portable variant does not modify the registry in the future. As a result, some features may **not** work properly:
...
Tauri 2 is currently in beta, and when it is released, we will introduce more package support and potentially Flatpak support.
...
<Tabs>
  <TabItem label="Stable (need to compile)">```bash paru -Syu clash-nyanpasu ```</TabItem>
  <TabItem label="Stable (binaries)">```bash paru -Syu clash-nyanpasu-bin ```</TabItem>
  <TabItem label="Development (need to compile)">```bash paru -Syu clash-nyanpasu-git ```</TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="User Directory">
    ```bash mkdir -p ~/.local/share/icons/clash-nyanpasu wget -O
    ~/.local/share/icons/clash-nyanpasu/clash-nyanpasu.png
    https://raw.githubusercontent.com/libnyanpasu/clash-nyanpasu/main/frontend/nyanpasu/src/assets/image/logo-box.png
    ```
  </TabItem>
  <TabItem label="System Directory">
    ```bash sudo mkdir -p /usr/share/icons/clash-nyanpasu sudo wget -O
    /usr/share/icons/clash-nyanpasu/clash-nyanpasu.png
    https://raw.githubusercontent.com/libnyanpasu/clash-nyanpasu/main/frontend/nyanpasu/src/assets/image/logo-box.png
    ```
  </TabItem>
</Tabs>
...
<Tabs>
  ...
  <TabItem label="System Directory">
    ```bash sudo update-desktop-database /usr/share/applications ```
  </TabItem>
</Tabs>
